...
│       └── samples/         # 5 bundled demo clips (one per genre)
├── code/
│   ├── 1_explore.ipynb     # EDA
...
│   ├── eval_results.md     # Latest test-set evaluation summary, written by 4_predict.ipynb
│   └── images/             # Plots generated by 1_explore.ipynb, 3_model.ipynb, 4_predict.ipynb
...
├── models/
│   └── best_model.pth      # Trained checkpoint (git-ignored; produced by 3_model.ipynb)
└── scripts/
...
A small local web app for trying the trained model interactively: upload a track (or click one of 5 bundled samples), and see the model's top-3 genre guesses plus the mel spectrogram it actually classified. See [app/README.md](app/README.md) for the full architecture — how it connects `3_model.ipynb`'s checkpoint and `code/common.py`'s pipeline through to the browser.

- `app/main.py` — FastAPI backend. `POST /predict` accepts an uploaded `.wav`/`.mp3`/`.flac`/`.ogg`/`.m4a` file and returns all 10 genre probabilities plus a spectrogram image as JSON. Serves the frontend at `/`.
...
- `app/static/index.html` — single-page frontend, no build step: sample-track chips, drag-and-drop upload, inline audio playback, a "hero" result for the top pick with two smaller runner-ups, and the analyzed clip's mel spectrogram.
- `app/static/samples/` — 5 bundled demo clips (blues, classical, hiphop, metal, reggae) generated by `scripts/generate_sample_clips.py`, so a live demo doesn't depend on someone having a file ready.

**Prerequisite:** `models/best_model.pth` must exist (run `3_model.ipynb` first). It's a local build artifact, not committed to git — each clone must run the notebook to produce its own copy.
